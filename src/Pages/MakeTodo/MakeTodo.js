...
const MakeTodo = () => {
  ...
  const createTodoList = (e) => {
    ...
    fetch(`https://cryptic-ridge-60527.herokuapp.com/todo`, {
      method: 'POST',
      headers: {
  ...
  return (
    <div>
      <h2 className="text-purple-500 font-bold uppercase text-center mt-20 mb-10 text-4xl">
        ...
        <div className="card w-full bg-base-100 ">
          <form className="w-full h-full" onSubmit={createTodoList}>
            <input
              ...
              name="taskName"
              className="w-full h-[40px] my-5 px-4 border-2"
              required
            />
            <input
              type="date"
            ...
            <input
              type="submit"
              value="Create"
              className="w-full h-[40px] my-5 px-4 border-2 bg-purple-500 btn"
            />
          </form>
        </div>
      </div>
